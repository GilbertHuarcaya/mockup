      ...
      {data.map((e) => {
        return (
          <View key={e.id} style={styles.row}>
            <Image source={{uri: e.picture}} style={{ width: 120, height: 120,     alignSelf: "center", marginRight: 15, }}/>
            <View style={styles.column}>
              <View>
                <Text style={{ fontWeight: "bold"}}>{e.about.split(" ").slice(0, 4).join(" ")}</Text>
                <Text>by {e.author}</Text>
              </View>
              <View>
                <View style={styles.rowTags}>
                  <Text style={styles.button} >{e.tags[0]}</Text>
                  <Text style={styles.button} >{e.tags[1]}</Text>
                </View>
                <View style={styles.rowComment}>
                  <Image source={{uri: e.picture}} style={{ width: 15, height: 15, marginRight: 15, }}/>
                  <Text>{e.comments.length} Comments</Text>
                </View>
              </View>
              {/* {e.comments.map((comment) => {
                return (
                  <Text>{comment.comment.split(" ").slice(0, 4).join(" ")}</Text>
                )})} */}
            </View>
          </View>
        )
